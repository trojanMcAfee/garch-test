...
res = spop.minimize(garch_mle, [mean, var, 0.1, 0.1], method='Nelder-Mead')

# Retrieving optimal parameters
mu, omega, alpha, beta = res.x

# Calculating realised and conditional volatility for the optimal parameters
resid = returns - mu
long_run = (omega / (1 - alpha - beta)) ** (1 / 2)
conditional = np.zeros(len(returns))
conditional[0] = long_run
...
plt.plot(prices.index[1:], abs(resid), label='Realized Volatility')
...
print(abs(resid)[-10:])
print("Old Conditional Volatility:")
print(conditional[-10:])
print("New Realized Volatility:")
print(new_realised)
print("New Conditional Volatility:")
print(new_conditional)
